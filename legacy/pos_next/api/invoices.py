...
def submit_invoice(invoice=None, data=None):
	...
	try:
		...
		if redeemed_customer_credit and not invoice_doc.payments:
			invoice_doc.flags.DyPOS_redeemed_customer_credit = flt(redeemed_customer_credit)
		...
		if invoice_doc.get("is_return") and invoice_doc.get("return_against"):
			from DyPOS.DyPOS.doctype.wallet_transaction.wallet_transaction import (
		...
		if redeemed_customer_credit and customer_credit_dict:
			try:
				from DyPOS.api.credit_sales import redeem_customer_credit
...
def apply_offers(invoice_data, selected_offers=None):
	...
	try:
		...
		# deferred by the per-item engine (see DyPOS.overrides.pricing_rule) and
