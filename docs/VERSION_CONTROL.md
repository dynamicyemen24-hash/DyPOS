# DyPOS - Version Control System

This document explains the version control mechanism implemented in DyPOS.

## Overview

DyPOS uses a comprehensive version control system that tracks both application versions and build versions for effective cache busting and version management.

## Version Types

### 1. Application Version
- Defined in: `DyPOS/__init__.py`
...
- Stored in: `DyPOS/public/pos/version.json`
...
2. **Version File** (`DyPOS/public/pos/version.json`):
   ```json
   {
...
3. **Backend Integration** (`DyPOS/utils.py`):
...
4. **Hooks Integration** (`DyPOS/hooks.py`):
   ```python
   _asset_version = get_build_version()
   # app_include_js = f"/assets/DyPOS/js/app.js?v={_asset_version}"
   ```
...
```bash
cd POS
DyPOS_BUILD_VERSION=1.2.3 yarn build
```
...
# Via Python
python3 -c "import sys; sys.path.insert(0, 'apps/DyPOS'); from DyPOS.utils import get_build_version; print(get_build_version())"
```
...
1. **Check version.json exists**:
   ```bash
   ls -la DyPOS/public/pos/version.json
   ```
...
```python
from DyPOS.utils import get_build_version, get_app_version
...
- **Location**: DyPOS uses `public/pos/` for build assets
- **Fallback**: DyPOS checks manifest.webmanifest mtime as additional fallback
- **Script**: DyPOS includes automated version bump script
- **Documentation**: DyPOS has comprehensive version management guide
