...
// Single CSP source: _headers AND the Worker-read module below share it.
...
// Worker-read CSP module: with a custom Worker script, Cloudflare ignores
// _headers for Worker-handled responses, so the Worker attaches the policy
// itself (see worker.js). Generated per build — hashes always match.
writeFileSync(join(OUT, 'csp.mjs'), `// Auto-generated by build-pages-site.mjs — do not edit.\nexport const CSP_POLICY = ${JSON.stringify(cspValue)};\n`, 'utf8');
...
if (!headers.includes(cspValue)) {
  console.error('[pages-site] FATAL: Worker CSP module diverged from _headers.');
  process.exit(1);
}
