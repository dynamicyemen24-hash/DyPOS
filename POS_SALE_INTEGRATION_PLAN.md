...
export function useItems(posProfile) {
  ...
  const itemsResource = createResource({
    url: 'DyPOS.api.invoices.get_items',
    params: {
...
- ✅ Create `DyPOS/api/invoices.py`
- ✅ Create `DyPOS/api/items.py`
...
- Wrapper API in `DyPOS` for clean separation
...
This integration will transform DyPOS from shift management to a full-featured POS system while maintaining code quality and design consistency.
