...
const DEFAULT_PRINT_FORMAT = "DyPOS Receipt"
...
function formatCurrency(amount) {
	// Precision-aware, locale-aware numeric formatting (receipt layout keeps
	// bare numbers; the currency label lives in the receipt header).
...
export function buildReceiptHTML(invoiceData) {
	...
	return `
			<div class="receipt">
				<div class="header">
					<div class="company-name">${invoiceData.company || "DyPOS"}</div>
				...
				<div class="footer">
					...
					${
						invoiceData.footer
							? ""
							: `<div style="font-size: 10px;">${__("Powered by")} <strong>DyPOS</strong></div>`
					}
				</div>
			</div>`
}
...
export async function printInvoice(
	...
	// Hydrate local-only invoices (offline) before printing; fall back gracefully
	let hydratedInvoice = null
	try {
		...
		const doctype = hydratedInvoice.doctype || "Sales Invoice"
		const format = printFormat || DEFAULT_PRINT_FORMAT

		const params = new URLSearchParams({
			doctype,
			name: hydratedInvoice.name,
			format,
		...
		if (
			isLocalOnlyInvoiceName(hydratedInvoice?.name) &&
...
export async function printInvoiceByName(
	...
	const invoiceDoc = await call("DyPOS.api.invoices.get_invoice", {
...
export async function printWithSilentFallback(invoiceData, printFormat = null) {
	const hydratedInvoice = await hydrateLocalOnlyInvoice(invoiceData)
	const invoiceName = hydratedInvoice?.name
	...
	if (
		...
		try {
			await silentPrintInvoiceFromDoc(hydratedInvoice)
		...
		try {
			printInvoiceCustom(hydratedInvoice)
