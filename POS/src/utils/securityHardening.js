...
function updateSessionActivity() {
	try {
		localStorage.setItem(
			"dypos_session_last_activity",
			new Date().toISOString(),
		)
		localStorage.setItem(
			"dypos_session_expiry",
			new Date(Date.now() + SESSION_ABSOLUTE_TIMEOUT_MS).toISOString(),
		)
	} catch (error) {
...
function isTokenExpiringSoon() {
	...
	const expiresAt = new Date(expiry).getTime()
	return (
		Number.isNaN(expiresAt) || Date.now() > expiresAt - TOKEN_RENEW_THRESHOLD_MS
	)
}
...
function registerSession(deviceId) {
	try {
		const existing = JSON.parse(
			localStorage.getItem("dypos_active_sessions") || "[]",
		)
		const now = Date.now()
		const active = existing.filter(
			(session) => now - session.registeredAt < SESSION_ABSOLUTE_TIMEOUT_MS,
		)
		active.push({ deviceId, registeredAt: now })
		localStorage.setItem(
			"dypos_active_sessions",
			JSON.stringify(active.slice(-MAX_CONCURRENT_SESSIONS)),
		)
		return true
	} catch (error) {
...
export function isSyncEvent(eventType) {
	return [
		SECURITY_EVENT_TYPES.SYNC_RETRY,
		SECURITY_EVENT_TYPES.SYNC_CONFLICT,
	].includes(eventType)
}
