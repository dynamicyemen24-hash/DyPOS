...
class RateLimitMetrics {
	...
	recordAttempt(key, allowed, retryAfterMs) {
		...
		this.serverEnforcements = this.serverEnforcements.filter(
	...
	getMetrics(key = null) {
		...
		return {
			...
			blockRate:
				attempts.length > 0 ? (blocks.length / attempts.length) * 100 : 0,
			avgResponseTime:
				enforcements.length > 0
					? enforcements.reduce((sum, e) => sum + e.responseTime, 0) /
						enforcements.length
					: 0,
			recentBlocks: blocks.filter(
				(b) => b.timestamp > Date.now() - 5 * 60 * 1000,
			).length,
		}
	}
}
	...
	async checkLimit(key, windowMs, maxAttempts) {
		...
		try {
			...
			const response = await fetch(
				`${SERVER_ENDPOINT}?key=${encodeURIComponent(key)}&windowMs=${windowMs}&maxAttempts=${maxAttempts}`,
				{
					method: "GET",
					headers: {
			...
			const result = {
				...
				resetTime:
					response.headers.get("X-RateLimit-Reset") || Date.now() + windowMs,
				serverEnforced: true,
				responseTime,
			}
			...
			log.warn?.(
				"Server rate limit check failed, falling back to client-side",
				error,
			)
...
export function createRateLimiter({
	...
	function checkServerEnforcement() {
		...
		return serverRateLimitClient.checkLimit(
			key,
			windowMs,
	...
	function check() {
		...
		if (serverEnforcement) {
			...
			if (serverResult) {
				if (!serverResult.serverEnforced) {
					...
					globalMetrics.recordBlock(
						key,
						serverResult.retryAfterMs || 0,
						"server_enforced",
					)
					handleRateLimited({
						key,
		...
		const remainingAttempts = Math.max(
			0,
			currentThreshold - state.attempts.length,
		)
		...
		if (state.attempts.length >= currentThreshold) {
			...
			const escalatedLockout = Math.min(
				lockoutMs * 2 ** (attemptCount - currentThreshold),
				maxLockoutMs,
			)
	...
	function recordFailure() {
		...
		if (adaptive) {
			...
			if (newThreshold !== state.adaptiveThreshold) {
				...
				globalMetrics.recordAdaptiveAdjustment(
					key,
					oldThreshold,
					newThreshold,
					"failure_pattern",
				...
				log.info?.(
					`Rate limit adaptive threshold adjusted for ${key}: ${oldThreshold} -> ${newThreshold}`,
				)
			}
		}
	...
	function recordSuccess() {
		...
		if (adaptive && state.adaptiveThreshold !== maxAttempts) {
			...
			log.info?.(
				`Rate limit adaptive threshold reset for ${key}: ${oldThreshold} -> ${maxAttempts}`,
			)
		}
