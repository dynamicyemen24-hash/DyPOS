...
function getCacheVersion() {
	...
	const storedHash = localStorage.getItem("DyPOS_cache_structure_hash")
	const storedVersion = Number.parseInt(
		localStorage.getItem("DyPOS_cache_version") || "1",
	)
	...
	if (storedHash !== structureHash.toString()) {
		...
		localStorage.setItem("DyPOS_cache_structure_hash", structureHash.toString())
		localStorage.setItem("DyPOS_cache_version", newVersion.toString())
		return newVersion
	}
...
export const initMemoryCache = async () => {
	try {
		...
		if (storedVersion !== CACHE_VERSION) {
			log.debug("Cache version mismatch, clearing cache...")
			await clearAllCache()
...
export const cacheItemsFromServer = async (posProfile) => {
	try {
		log.debug("Fetching items from server...")
		...
		if (response.message && Array.isArray(response.message)) {
			...
			log.debug(`Fetched ${processedItems.length} items from server`)
...
export const getCachedItem = async (itemCode) => {
	try {
		...
		log.error("Error getting cached item:", error)
		return null
	}
}
...
export const getCachedCustomer = async (customerName) => {
	try {
		...
		log.error("Error getting cached customer:", error)
		return null
	}
}
...
export async function cachePaymentMethodsFromServer(posProfile) {
	try {
		const result = await call("DyPOS.api.pos_profile.get_payment_methods", {
			pos_profile: posProfile,
		})
...
export async function cacheSalesPersonsFromServer(posProfile) {
	try {
		const result = await call("DyPOS.api.pos_profile.get_sales_persons", {
			pos_profile: posProfile,
		})
		...
		log.debug(`Cached ${salesPersons.length} sales persons for ${posProfile}`)
