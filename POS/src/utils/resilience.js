...
const circuits = new Map<string, CircuitBreakerConfig>()

/**
 * Get or create a circuit breaker
 * @param {string} name - Unique name for the circuit
 * @param {Partial<CircuitBreakerConfig>} options
 ...
 * @param {{ fallback?: () => T | Promise<T>; onOpen?: () => void }} options
 * @returns {Promise<T>}
 */
...
const DEFAULT_RETRY_OPTIONS = {
	...
	jitter: 0.3,
	retryable: (error) => {
		// Network errors, timeouts, 5xx, rate limits
	...
	for (let attempt = 1; attempt <= opts.maxAttempts; attempt++) {
		try {
			...
			const delay = Math.min(
				opts.baseDelay * Math.pow(2, attempt - 1) * (1 + (Math.random() - 0.5) * 2 * opts.jitter),
				opts.maxDelay,
			)
...
export function withTimeout(promise, ms, operationName = "operation") {
	return Promise.race([
		...
		new Promise<T>((_, reject) => {
			...
			// Cleanup if promise resolves first
...
const bulkheads = new Map<string, BulkheadConfig>()

/**
 * Get or create bulkhead
 * @param {string} name
 * @param {{ maxConcurrent?: number; queueLimit?: number }} options
 */
