	...
	for (let i = 0; i < itemCodes.length; i += BATCH_SIZE) {
		...
		try {
			const response = await call(
				"DyPOS.api.items.get_batch_serial_data_for_items",
				{
...
export const useItemSearchStore = defineStore("itemSearch", () => {
	...
	const itemGroupsResource = createResource({
		url: "DyPOS.api.items.get_item_groups",
		makeParams() {
			return {
	...
	const brandsResource = createResource({
		url: "DyPOS.api.items.get_brands",
		makeParams() {
			return {
				pos_profile: posProfile.value,
			}
	...
	const searchByBarcodeResource = createResource({
		url: "DyPOS.api.items.search_by_barcode",
		auto: false,
	})
	...
	async function refreshItem(itemCode, profile = posProfile.value) {
		...
		try {
			const items = await call("DyPOS.api.items.get_items", {
				pos_profile: profile,
	...
	const filteredItems = computed(() => {
		...
		// Also inject favorite status (multi-list favorites support)
		const favStore = useFavoritesStore()
		const favListMap = new Map()
		for (const favList of favStore.lists) {
			for (const code of favList.itemCodes) {
				if (!favListMap.has(code)) favListMap.set(code, [])
				favListMap.get(code).push(favList.id)
			}
		}
		...
		const itemsWithStock = list.map((item) => {
			...
			// Favorite status across all custom lists
			const favListIds = favListMap.get(item.item_code) || []

			// Return item with updated stock quantities + favorite metadata
			return {
				...item,
				actual_qty: displayStock,
				stock_qty: displayStock,
				original_stock: originalStock,
				_favorite: favListIds.length > 0,
				_favoriteListIds: favListIds,
			}
		})
		...
		if (sortBy.value) {
			itemsWithStock.sort((a, b) => {
				...
				switch (sortBy.value) {
					...
					case "favorites":
						// Favorited items first, then alphabetical by name
						if (a._favorite !== b._favorite) {
							compareResult = a._favorite ? -1 : 1
						} else {
							compareResult = (a.item_name || "").localeCompare(
								b.item_name || "",
							)
						}
						break
	...
	// ========================================================================
	// FAVORITES INTEGRATION (multi-list, custom names)
	// ========================================================================

	/**
	 * Toggle favorite status of an item in a given list (or default list).
	 * @param {string} itemCode
	 * @param {string} [listId] - Target list; defaults to first list
	 * @returns {{ok: boolean, favorited?: boolean, reason?: string}}
	 */
	function toggleFavoriteItem(itemCode, listId) {
		if (!itemCode) return { ok: false, reason: "invalid-code" }
		const fav = useFavoritesStore()
		const targetListId = listId || fav.getFirstList()?.id
		if (!targetListId) return { ok: false, reason: "no-list" }

		const currentListIds = fav.listIdsForCode(itemCode)
		if (currentListIds.includes(targetListId)) {
			const removed = fav.removeFromList(itemCode, targetListId)
			return removed
				? { ok: true, favorited: false }
				: { ok: false, reason: "remove-failed" }
		}
		const res = fav.addToListView({ code: itemCode, listId: targetListId })
		return res.ok ? { ok: true, favorited: true } : res
	}

	/** Add an item to a specific favorites list */
	function addToFavorites(itemCode, listId) {
		if (!itemCode) return { ok: false, reason: "invalid-code" }
		const fav = useFavoritesStore()
		const targetListId = listId || fav.getFirstList()?.id
		if (!targetListId) return { ok: false, reason: "no-list" }
		return fav.addToListView({ code: itemCode, listId: targetListId })
	}

	/** Remove an item from a specific favorites list */
	function removeFromFavorites(itemCode, listId) {
		const fav = useFavoritesStore()
		const targetListId = listId || fav.getFirstList()?.id
		if (!targetListId) return false
		return fav.removeFromList(itemCode, targetListId)
	}

	/** Get list ids an item belongs to */
	function getItemFavoriteListIds(itemCode) {
		const fav = useFavoritesStore()
		return fav.listIdsForCode(itemCode)
	}

	/**
	 * Reactive set of all favorited item codes — use in templates to
	 * highlight favorites without re-running filteredItems.
	 */
	const favoriteCodesSet = computed(() => {
		const fav = useFavoritesStore()
		return new Set(fav.allCodes)
	})
	...
	async function loadAllItems(profile, forceServerFetch = false) {
		...
		try {
			...
			const countPromise = !offline
				? call("DyPOS.api.items.get_items_count", {
						pos_profile: profile,
						show_variants_as_items: getShowVariantsFlag(),
					})
			...
			if (hasFilters && selectedItemGroup.value) {
				...
				const response = await call("DyPOS.api.items.get_items", {
					pos_profile: profile,
					search_term: "",
	...
	async function fetchItemsForGroup(
		...
		try {
			...
			if (groupsToFetch.length > 1) {
				const response = await call("DyPOS.api.items.get_items_bulk", {
					pos_profile: profile,
				...
				try {
					if (hasFilters && groupsToSync.length > 0) {
						...
						for (let i = 0; i < PARALLEL_REQUESTS; i++) {
							...
							offsets.push(offset)
							promises.push(
								call("DyPOS.api.items.get_items_bulk", {
									pos_profile: profile,
									start: offset,
									limit: batchSize,
									show_variants_as_items: getShowVariantsFlag(),
					...
					const backoffMs = Math.min(2000 * 2 ** (consecutiveErrors - 1), 30000)
					log.warn(
	...
	async function searchItems(term) {
		...
		return new Promise((resolve) => {
			searchDebounceTimer = setTimeout(async () => {
				...
				try {
					// Arabic-aware search: fold diacritics/hamza/digits so
					// "بَيْت"/"بيت"/"بیت" and "123"/"١٢٣" all match.
	...
	async function setSelectedItemGroup(group) {
		...
		if (posProfile.value) {
			...
			try {
				...
				const countPromise = call("DyPOS.api.items.get_items_count", {
					pos_profile: posProfile.value,
	...
	async function setPosProfile(profile, autoLoadItems = true) {
		...
		try {
			...
			const data = await call("DyPOS.api.pos_profile.get_pos_profile_data", {
				pos_profile: profile,
			})
	...
	return {
		...
		// ========================================================================
		// FAVORITES ACTIONS - Multi-list, custom names
		// ========================================================================
		toggleFavoriteItem,
		addToFavorites,
		removeFromFavorites,
		getItemFavoriteListIds,
		favoriteCodesSet,
