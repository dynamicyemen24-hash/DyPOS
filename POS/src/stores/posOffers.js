...
export const usePOSOffersStore = defineStore("posOffers", () => {
	...
	function setOneTimeContext({ identified = false, redeemedRules = [] } = {}) {
		...
		redeemedOneTimeRules.value = Array.isArray(redeemedRules)
			? redeemedRules
			: []
	}
	 ...
	 */
	async function loadOneTimeContextForCustomer(
		customerName,
		{ isWalkIn = false } = {},
	) {
		...
		if (!isOffline()) {
			try {
				const resp = await call(
					"DyPOS.api.offers.get_customer_one_time_redemptions",
					{
		...
		if (offer?.one_time_per_customer) {
			...
			if (redeemedOneTimeRules.value.includes(offer.name)) {
				return {
					eligible: false,
					reason: __(
						"This one-time offer has already been used by this customer",
					),
				}
			}
		}
	...
	async function ensureOffersFetched(posProfile) {
		...
		fetchPromise = (async () => {
			try {
				...
				const response = await call("DyPOS.api.offers.get_offers", {
