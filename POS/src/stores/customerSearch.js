...
export const useCustomerSearchStore = defineStore("customerSearch", () => {
	...
	// Customer composable for cache-first loading
	const { loadCustomers, cacheReady, searchCustomers } = useCustomers()
	...
	async function loadAllCustomers(posProfile, forceReload = false) {
		...
		// Use composable for initial cache load
		if (!loading.value) {
			await loadCustomers()
		}
		...
		loading.value = true
		try {
			// Step 1: Load from IndexedDB cache (instant display) - composable already did this
	...
	onCustomerUpdate(async (data) => {
		...
		log.debug("Customer update via real-time:", data)
