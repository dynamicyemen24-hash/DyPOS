...
const pinExpiry = computed(() =>
	storedPinExpiry.value ? new Date(storedPinExpiry.value) : null,
)
...
function loadPinState() {
	try {
		...
		if (!raw) {
			storedPinHash.value = null
			storedPinExpiry.value = 0
			pinUserEmail.value = null
			return
		}
		const parsed = JSON.parse(raw)
		if (parsed?.hash && parsed?.email && parsed?.expiry) {
			storedPinHash.value = parsed.hash
			pinUserEmail.value = parsed.email
			storedPinExpiry.value = parsed.expiry
		} else {
			clearPin()
		}
	} catch {
		...
		const hash = await window.crypto.subtle
			...
			.then((key) =>
				window.crypto.subtle.deriveBits(
					{
						name: "PBKDF2",
						salt: new TextEncoder().encode("dypos-pin-salt-v1"),
						iterations: 100000,
						hash: "SHA-256",
					},
					key,
...
async function pinLogin(pinCode) {
	if (!storedPinHash.value)
		return { success: false, error: "لا يوجد PIN محفوظ" }
	if (isLocked.value)
		return {
			success: false,
			error: `الحساب مقفل — حاول بعد ${getLockRemainingSeconds()} ثانية`,
		}
