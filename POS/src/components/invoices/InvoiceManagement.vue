									...
									>
										...
										<div class="p-4">
											<div class="grid grid-cols-3 gap-4 mb-4">
												<div
													class="text-center p-3 bg-indigo-50 rounded-lg border border-indigo-100"
												>
							...
							<div v-if="activeTab === 'history'">
								...
								<div v-else class="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
									...
									>
										...
										<div
											class="bg-gradient-to-r from-indigo-50 to-indigo-50 px-5 py-4 border-b border-gray-200"
										>
...
// Returns (is_return) are excluded at the source so both the FilterBar and the
// history list operate on the exact same data in a single filter instance.
const historyInvoicesRef = computed(() => {
...
// Filtered history using the single hoisted filter instance
// (previously re-created the whole composable on every recomputation)
const filteredHistoryInvoices = computed(
	() => invoiceFilters.filteredInvoices.value,
)
...
async function loadUnpaidSummary() {
	...
	try {
		const result = await call("DyPOS.api.partial_payments.get_unpaid_summary", {
			pos_profile: props.posProfile,
		})
...
async function selectInvoiceForPayment(invoice) {
	...
	try {
		...
		const details = await call(
			"DyPOS.api.partial_payments.get_partial_payment_details",
			{
...
async function handlePaymentCompleted(paymentData) {
	...
	try {
		await call("DyPOS.api.partial_payments.add_payment_to_partial_invoice", {
