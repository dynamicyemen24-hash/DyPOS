		...
		<template #body-content>
			<div class="flex flex-col gap-3 md:gap-6">
				<div v-if="closingDataResource.loading" class="text-center py-8 md:py-12">
					<div
						class="inline-block animate-spin rounded-full h-12 w-12 md:h-16 md:w-16 border-b-4 border-indigo-600"
					></div>
				...
				<div v-else-if="closingData" class="flex flex-col gap-3 md:gap-6">
					...
					>
						...
						<div class="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
							...
							<div
								class="text-start bg-indigo-50 border border-indigo-200 rounded-lg p-3 md:p-4"
							>
								<div class="text-indigo-600 text-xs uppercase font-medium mb-1">
									{{ __("Gross Sales") }}
								</div>
								<div
									class="text-lg md:text-2xl font-bold text-indigo-900 mb-0.5 md:mb-1 truncate"
								>
...
const collectedAmount = computed(() => {
	if (!closingData.value) return 0
	return Number(
		closingData.value.collected_amount ?? sumTransactions("collected_amount"),
	)
})
...
const outstandingTotal = computed(() => {
	if (!closingData.value) return 0
	return Number(
		closingData.value.outstanding_total ??
			sumTransactions("outstanding_amount"),
	)
})
