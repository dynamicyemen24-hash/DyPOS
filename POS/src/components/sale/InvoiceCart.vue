...
<template>
	<div class="flex flex-col h-full bg-white">
					...
					<div class="flex items-stretch gap-2">
						...
						>
							...
							<div class="flex items-center gap-2 min-w-0 flex-1 px-1.5 py-1">
								<div
									class="w-8 h-8 bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-full flex items-center justify-center flex-shrink-0"
								>
									<svg
							...
							<div class="flex items-center gap-0.5 flex-shrink-0" @click.stop>
								<button
									...
									class="w-7 h-7 flex items-center justify-center text-indigo-500 hover:bg-indigo-50 active:bg-indigo-100 rounded-lg transition-colors touch-manipulation"
					...
					<div class="flex gap-1.5">
						...
						<div class="relative flex-1">
							...
							>
								...
								<div
									v-else
									class="animate-spin rounded-full h-3.5 w-3.5 border-b-2 border-indigo-500"
								></div>
							</div>
							...
							<input
								...
								class="w-full h-10 ps-9 pe-3 text-xs border border-gray-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm transition-shadow"
				...
				>
					...
					>
						<button
							...
							:class="[
								'w-full text-start px-2 py-1.5 flex items-center gap-1.5 border-b border-gray-100 last:border-0 touch-manipulation select-none cursor-pointer active:bg-indigo-200',
								index === selectedIndex
									? 'bg-indigo-100'
						...
						>
							<div
								class="w-6 h-6 bg-indigo-100 rounded-full flex items-center justify-center flex-shrink-0 pointer-events-none"
							>
				...
				<div
					...
					:class="[
						...
						item.is_free_item
							...
							: 'bg-white border-gray-200 hover:border-indigo-300 hover:shadow-md active:scale-[0.99] cursor-pointer group',
					]"
				>
							...
							<div class="flex items-center justify-between gap-1.5">
								<div class="flex items-center gap-1.5">
									...
									>
										<!-- Serial count badge -->
										<div
											class="flex items-center bg-indigo-50 border border-indigo-200 rounded px-1.5 h-6 sm:h-7"
										>
											<FeatherIcon
												name="hash"
												class="w-3 h-3 text-indigo-500 me-0.5"
											/>
											<span
												class="text-xs sm:text-sm font-bold text-indigo-700"
												>{{ item.quantity }}</span
											>
										</div>
										<!-- Edit button -->
										<button
											...
											class="flex items-center justify-center w-6 h-6 sm:w-7 sm:h-7 bg-indigo-500 hover:bg-indigo-600 active:bg-indigo-700 text-white rounded transition-colors shadow-sm"
									...
									<div class="relative group/uom" @click.stop>
											...
											:class="[
												...
												item.is_resolved_barcode
													...
													: item.item_uoms && item.item_uoms.length > 0
														? 'bg-indigo-500 text-white border border-indigo-400 hover:bg-indigo-600 active:scale-95 cursor-pointer'
										...
										<div
											v-if="
												...
												item.item_uoms.length > 0
											"
											class="absolute top-full start-0 mt-0.5 bg-white border border-indigo-300 rounded shadow-xl z-50 min-w-full overflow-hidden"
										>
											<button
												type="button"
								...
								<div class="text-end flex-shrink-0">
									<div
										class="text-xs sm:text-sm font-bold text-indigo-600 leading-none"
									>
										{{
											formatCurrency(
		...
		<div class="p-1.5 sm:p-2 bg-white border-t border-gray-200">
			...
			<!-- Grand Total -->
			<div class="bg-gradient-to-r from-indigo-50 to-indigo-50 rounded-lg p-2.5 mb-1.5">
				<div class="flex items-center justify-between">
					...
					<span
						role="status"
						aria-live="polite"
						class="text-lg sm:text-xl font-extrabold text-indigo-600 text-center min-w-[60px]"
					>
				...
				<button
					...
					:class="[
						...
						items.length === 0
							...
							: 'bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 shadow-lg hover:shadow-xl active:scale-[0.98]',
					]"
...
// Debounced mirror of customerSearch used only by the results filter, so the
// O(allCustomers) in-memory scan doesn't run on every keystroke.
const customerSearchDebounce = ref(0)
const effectiveCustomerSearch = ref("")
watch(customerSearch, (val) => {
	clearTimeout(customerSearchDebounce.value)
	customerSearchDebounce.value = setTimeout(() => {
		effectiveCustomerSearch.value = val
	}, 120)
})
 ...
 *
 * @endpoint DyPOS.api.offers.get_active_coupons
 */
const giftCardsResource = createResource({
	url: "DyPOS.api.offers.get_active_coupons",
	makeParams() {
		return {
...
const customerResults = computed(() => {
	const searchValue = effectiveCustomerSearch.value.trim().toLowerCase()
