		...
		<template #body-content>
			...
			<template v-else>
				...
				>
					<div class="relative">
						...
						>
							...
							<div
								v-else
								class="animate-spin rounded-full h-4 w-4 border-b-2 border-indigo-500"
							></div>
						</div>
						<!-- Search Input -->
						<input
							...
							class="stock-search-input w-full ps-10 pe-10 py-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
				...
				<div
					...
					class="py-2 px-3 bg-indigo-50 border border-indigo-200 rounded-lg mb-4"
				>
					<div class="flex items-center gap-2">
						<!-- Item Image -->
						<div
							class="w-8 h-8 bg-white rounded flex items-center justify-center overflow-hidden border border-indigo-200 flex-shrink-0"
						>
							<img
						...
						<button
							...
							class="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-indigo-100 rounded transition-colors flex-shrink-0"
						...
						<div class="flex items-center justify-between mb-3">
							...
							<div class="flex gap-2">
								<button
									...
									class="text-xs px-2 py-1 text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 rounded transition-colors"
								>
									{{ __("Select All") }}
								</button>
								<button
				...
				<div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
					...
					>
						...
						<div
							...
							"
							class="text-indigo-600 font-medium mt-1"
						>
	...
	async (newVal) => {
		if (newVal) {
			...
			if (isSearchMode.value) {
				...
				try {
					const itemResponse = await call("DyPOS.api.items.get_items", {
...
async function loadVariants() {
	...
	try {
		const response = await call("DyPOS.api.items.get_item_variants", {
	...
	try {
		...
		if (selectedVariants.value.length > 0) {
			...
			const response = await call(
				"DyPOS.api.items.get_item_warehouse_availability",
				{
