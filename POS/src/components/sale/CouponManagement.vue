						...
						<Card>
							<div class="p-5">
								<div class="flex items-center gap-2 mb-4">
									<FeatherIcon name="info" class="w-4 h-4 text-indigo-600" />
...
const couponsResource = createResource({
	url: "DyPOS.api.promotions.get_coupons",
	makeParams() {
		return {
			company: props.company,
...
const couponDetailsResource = createResource({
	url: "DyPOS.api.promotions.get_coupon_details",
	makeParams() {
...
const createCouponResource = createResource({
	url: "DyPOS.api.promotions.create_coupon",
	makeParams() {
...
const updateCouponResource = createResource({
	url: "DyPOS.api.promotions.update_coupon",
	makeParams() {
		return {
...
const toggleCouponResource = createResource({
	url: "DyPOS.api.promotions.toggle_coupon",
	makeParams() {
		return { coupon_name: selectedCoupon.value?.name }
	},
	auto: false,
...
const deleteCouponResource = createResource({
	url: "DyPOS.api.promotions.delete_coupon",
	makeParams() {
		return { coupon_name: selectedCoupon.value?.name }
	},
	auto: false,
...
function formatCurrency(amount) {
	return formatCurrencySafe(amount, props.currency)
}
