<template>
	<div class="flex flex-col h-full bg-gray-50">
		...
		<div v-if="cacheSyncing" class="px-1.5 sm:px-3 py-1 bg-indigo-50 border-b border-indigo-200">
			<div
				class="flex items-center justify-center gap-2 text-[10px] sm:text-xs text-indigo-700"
			>
				...
				<div class="flex-1 relative min-w-0">
					...
					<input
						...
						:class="[
							...
							autoAddEnabled
								? 'border-indigo-400 bg-indigo-50 focus:ring-2 focus:ring-indigo-500 focus:border-transparent'
								: scannerEnabled
					...
					<div class="absolute inset-y-0 end-0 pe-1 sm:pe-2 flex items-center gap-0.5">
						...
						<button
							...
							:class="[
								...
								autoAddEnabled
									? 'bg-indigo-100 hover:bg-indigo-200 active:bg-indigo-300 text-indigo-700'
									: 'hover:bg-gray-100 active:bg-gray-200 text-gray-600',
							]"
							:title="
								autoAddEnabled
...
<!-- Smart recent scans quick-strip (session-local behaviour) -->
		<div
			v-if="recentItemsQuick.length > 0 && !searchTerm && !loading"
			class="px-1.5 sm:px-3 py-1.5 bg-white border-b border-indigo-100"
		>
			<div class="flex items-center gap-1 sm:gap-1.5 overflow-x-auto scrollbar-hide">
				<span
					class="text-[10px] sm:text-[11px] font-semibold text-indigo-600 whitespace-nowrap"
				>
					{{ __("Recently scanned") }}:
				</span>
				<button
					v-for="item in recentItemsQuick"
					:key="item.item_code"
					type="button"
					@click="handleRecentItemClick(item)"
					class="flex items-center gap-1 px-2 sm:px-2.5 py-1 rounded-lg text-[10px] sm:text-[11px] font-medium whitespace-nowrap bg-indigo-50 text-indigo-700 hover:bg-indigo-100 active:bg-indigo-200 transition-[background-color] duration-75 touch-manipulation"
				...
				>
					...
					<span class="max-w-[90px] sm:max-w-[130px] truncate">{{ item.item_name || item.name }}</span>
				</button>
			</div>
		</div>
		...
		>
			<div class="text-center py-8">
				...
				<div
					v-if="smartSearch.corrections.length"
					class="mt-3 flex flex-wrap items-center justify-center gap-1.5"
				>
					<span class="text-xs text-gray-500">{{ __("Did you mean") }}</span>
					<button
						v-for="corrected in smartSearch.corrections"
						:key="corrected"
						type="button"
						@click="applySearchCorrection(corrected)"
						class="px-2.5 py-1 text-xs font-semibold rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200 hover:bg-indigo-100 active:bg-indigo-200 transition-colors touch-manipulation"
					>
						{{ corrected }}
					</button>
				</div>
		...
		>
			<div
				...
				class="flex-1 overflow-y-auto p-1.5 sm:p-3 outline-none"
				style="min-height: 0"
				tabindex="0"
				role="grid"
				:aria-label="__('Items grid')"
				@keydown="gridKeys.handleKeydown"
			>
				<div
					v-if="virtualizing"
					:style="{ height: virtualGrid.offsetTop + 'px' }"
				></div>
				<div
				...
				>
					<div
						...
						:class="[
							'group relative bg-white border border-gray-200 rounded-lg p-1.5 sm:p-2.5 touch-manipulation transition-[border-color,box-shadow] duration-100 cursor-pointer hover:border-indigo-400 hover:shadow-md',
							gridKeys.isActive(idx) ? 'border-indigo-500 ring-2 ring-indigo-300' : '',
						]"
					>
			...
			>
				...
				>
					...
					<tbody class="bg-white divide-y divide-gray-200">
						<tr v-if="listOffsetTop > 0" class="bg-white">
							<td :colspan="6" class="p-0" :style="{ height: listOffsetTop + 'px' }"></td>
						</tr>
						<tr
							...
							@touchend.passive="getOptimizedClickHandler(item).touchend"
							@click="getOptimizedClickHandler(item).click"
							class="group cursor-pointer hover:bg-indigo-50 hover:shadow-md transition-[background-color,box-shadow] duration-100 touch-manipulation active:bg-indigo-100"
						>
							...
							<td class="px-2 sm:px-3 py-2 whitespace-nowrap w-[70px] sm:w-[100px]">
								<div class="text-xs sm:text-sm font-semibold text-indigo-600">
									{{ formatCurrency(item.rate || item.price_list_rate || 0) }}
								</div>
							</td>
							<td class="px-2 sm:px-3 py-2 whitespace-nowrap w-[70px] sm:w-[100px]">
						...
						<!-- Bottom spacer for windowed list (keeps scrollbar height accurate) -->
						<tr v-if="listBottomSpacer > 0" class="bg-white">
							<td :colspan="6" class="p-0" :style="{ height: listBottomSpacer + 'px' }"></td>
						</tr>
...
import { useSmartSuggestionsStore } from "@/composables/useSmartSuggestions"
...
// Smart recent-scan quick strip: resolves last scanned codes to item objects
const smartSuggestionsStore = useSmartSuggestionsStore()
const { recentScans } = storeToRefs(smartSuggestionsStore)
const recentItemsQuick = ref([])
let recentScanVersion = 0

async function refreshRecentQuick() {
	const codes = recentScans.value.slice(0, 6)
	if (!codes.length) {
		recentItemsQuick.value = []
		return
	}
	const version = ++recentScanVersion
	const resolved = await Promise.all(
		codes.map((code) => itemStore.getItem(code).catch(() => null)),
	)
	// Guard against races: only apply the most recent resolution
	if (version === recentScanVersion) {
		recentItemsQuick.value = resolved.filter(Boolean)
	}
}

watch(recentScans, refreshRecentQuick, { immediate: true })

function handleRecentItemClick(item) {
	selectItem(item, autoAddEnabled.value || scannerEnabled.value)
}
...
// Overlaid on the empty state when a search term returns zero results.
// Corrections come from the currently loaded catalog (item code/name/
// brand/group), normalized Arabic-first. Only ever runs after a search
// has actually completed, so it never races the store's own pipeline.
// ------------------------------------------------------------------
const SEARCH_DICTIONARY_MAX = 2000

const searchDictionary = computed(() => {
	...
	for (const it of items) {
		if (dict.length >= SEARCH_DICTIONARY_MAX) break
		addTerm(it.item_code)
		addTerm(it.item_name)
		addTerm(it.brand)
		addTerm(it.item_group)
	}
	return dict
})
...
// Recompute corrections only when a search finished with zero results.
watch(
	() => [
		itemStore.searching,
	...
	() => {
		...
		if (
			!term ||
			itemStore.searching ||
			(filteredItems.value?.length || 0) > 0
		) {
			smartSearch.reset()
			return
		}
		smartSearch.searchDebounced()
	},
	{ flush: "post" },
)

function applySearchCorrection(corrected) {
	smartSearch.reset()
	itemStore.setSearchTerm(corrected)
	focusSearchInput()
}

// ------------------------------------------------------------------
// VIRTUALIZED ITEM GRID + KEYBOARD NAVIGATION
// Large result sets (search returns up to ~500) are windowed to the
// visible rows only. Row height/columns are derived from the measured
// container width, so they stay accurate across breakpoints and RTL.
// Keyboard arrows navigate the visible window (grid-aware, RTL-aware).
// ------------------------------------------------------------------
const GRID_ROW_TEXT_EXTRA = 52 // image 1:1 + text lines under the square
const VIRTUALIZE_THRESHOLD = 40
const { isRTL } = useLocale()

const gridContainerWidth = ref(0)
const gridColumns = ref(4)
const gridColumnBreakpoints = [
	{ width: 1280, cols: 5 },
	{ width: 1024, cols: 4 },
	{ width: 768, cols: 3 },
	{ width: 640, cols: 2 },
...
// Scroll container refs. Declared here (before first use below) so the virtual
// grid / windowed list composables receive them without hitting the temporal
// dead zone of a later `const`.
const gridScrollContainer = ref(null)
const listScrollContainer = ref(null)

function measureGridContainer() {
	const el = gridScrollContainer.value
	return el?.clientWidth || el?.offsetWidth || 0
}

function updateGridColumns() {
	const width = measureGridContainer()
	if (!width) return
	gridContainerWidth.value = width
	let cols = 2
	for (const bp of gridColumnBreakpoints) {
		if (width >= bp.width) {
			cols = bp.cols
			break
		}
	}
	if (cols !== gridColumns.value) {
...
const gridRowHeight = computed(() => {
	...
	if (!width || !gridColumns.value) return 96
	const gap = 10
	const padding = 12
	const colWidth =
		(width - padding - gap * (gridColumns.value - 1)) / gridColumns.value
	return Math.max(80, Math.round(colWidth + GRID_ROW_TEXT_EXTRA))
})

const virtualizing = computed(
	() =>
		viewMode.value === "grid" &&
		displayedItems.value.length > VIRTUALIZE_THRESHOLD,
)

const virtualGrid = useVirtualGrid({
	total: computed(() => (virtualizing.value ? displayedItems.value.length : 0)),
	containerRef: gridScrollContainer,
	columns: gridColumns,
...
const visibleItems = computed(() => {
	...
	if (!virtualizing.value) return all
	return all.slice(virtualGrid.startIndex.value, virtualGrid.endIndex.value)
})

const virtualBottomSpacer = computed(() => {
	if (!virtualizing.value) return 0
	const endRow = Math.ceil(virtualGrid.endIndex.value / gridColumns.value)
	return Math.max(
		0,
		virtualGrid.totalHeight.value - endRow * gridRowHeight.value,
	)
})

// ------------------------------------------------------------------
// LIST VIEW WINDOWING
// Table mode previously mounted the entire result set (search can return
// up to ~500 rows) as one big <table>. It is now windowed with the same
// offsetTop/bottom-spacer technique used by the grid, reusing the shared
// useWindowedList composable. Display-only: cart/sale logic untouched.
// ------------------------------------------------------------------
const LIST_ROW_HEIGHT = 52

const listWindow = useWindowedList({
	total: computed(() =>
		viewMode.value === "list" ? displayedItems.value.length : 0,
	),
	containerRef: listScrollContainer,
	itemHeight: LIST_ROW_HEIGHT,
	overscan: 8,
})

const listVisibleItems = computed(() => {
	const all = displayedItems.value
	if (viewMode.value !== "list") return all
	return all.slice(listWindow.startIndex.value, listWindow.endIndex.value)
})

const listOffsetTop = computed(() =>
	viewMode.value === "list" ? listWindow.offsetTop.value : 0,
)

const listBottomSpacer = computed(() => {
	if (viewMode.value !== "list") return 0
	return Math.max(
		0,
		listWindow.totalHeight.value - listWindow.endIndex.value * LIST_ROW_HEIGHT,
	)
})

const gridKeys = useKeyboardListNavigation({
	count: () =>
		virtualizing.value
	...
	columns: gridColumns,
	onSelect: (index) => {
		const source = virtualizing.value
			? visibleItems.value
			: displayedItems.value
		const item = source[index]
		if (item) handleItemClick(item.item_code)
	},
	enabled: computed(() => viewMode.value === "grid"),
	dir: isRTL,
	wrap: true,
})

// Reset keyboard selection whenever the result set or view changes.
watch(
	() => [displayedItems.value?.length || 0, viewMode.value, searchTerm.value],
	() => {
		gridKeys.reset()
	},
	{ flush: "post" },
...
// Keep the measured column count in sync with the scroll container's width.
let gridWidthObserver = null

function bindGridWidthObserver() {
	if (gridWidthObserver) {
		gridWidthObserver.disconnect()
		gridWidthObserver = null
	}
	const container = gridScrollContainer.value
	if (!container || viewMode.value !== "grid") return
	updateGridColumns()
	if (typeof ResizeObserver !== "undefined") {
		gridWidthObserver = new ResizeObserver(() => updateGridColumns())
		gridWidthObserver.observe(container)
	}
}
...
// Scroll container refs are declared earlier in this setup block, before the
// virtual grid / windowed list composables consume them.
...
function selectItem(item, autoAdd = false) {
	...
	// ✅ نجاح الإضافة — سجل العنصر في تاريخ الاقتراحات الذكية لتحسين البحث المستقبلي
	smartSuggestionsStore.recordScan(item.item_code)
