<template>
	<Dialog v-model="show" :options="{ title: __('POS Shift History'), size: '7xl' }">
		<template #body-content>
			<div class="flex flex-col gap-6">
				<!-- Quick Summary Cards — totals come from the server (#11 fix) -->
				<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
					<div
						class="bg-indigo-50 border border-indigo-100 rounded-xl p-4 flex flex-col justify-between shadow-sm"
					>
						<span
							class="text-xs font-semibold text-indigo-600 uppercase tracking-wider"
							>{{ __("Total Sales") }}</span
						>
						<span class="text-2xl font-bold text-indigo-900 mt-1">{{
					...
					>
						<span
							class="text-xs font-semibold text-indigo-600 uppercase tracking-wider"
							>{{ __("Total Shifts") }}</span
						>
						<span class="text-2xl font-bold text-indigo-900 mt-1">{{
							serverTotals.total_shifts
						}}</span>
					</div>
					<div
						:class="[
							'rounded-xl p-4 flex flex-col justify-between shadow-sm border',
							serverTotals.total_cash_diff >= 0
					...
					>
						...
						<span
							...
							>{{ formatCurrency(serverTotals.total_cash_diff) }}</span
						>
					</div>
				</div>
				...
				<!-- Filters Section -->
				<div
					class="flex flex-col sm:flex-row items-end gap-3 bg-gray-50/50 p-4 rounded-xl border border-gray-100"
				>
					<div class="w-full sm:w-44">
						<label
							class="block text-[10px] font-bold text-gray-400 uppercase mb-1 ms-1"
							>{{ __("From Date") }}</label
						>
						<Input
							type="date"
				...
				<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
					...
					>
						...
						></div>
						<p
							class="mt-4 text-sm font-semibold text-gray-500 tracking-wide uppercase"
						>
							{{ __("Loading your history...") }}
						</p>
					</div>
					...
					>
						...
						<p
							class="mt-4 text-sm font-semibold text-gray-500 uppercase tracking-widest"
						>
							{{ __("No shifts found for this range") }}
						</p>
					</div>

					<div v-else class="overflow-x-auto max-h-[50vh]">
						<table class="w-full text-start text-sm whitespace-nowrap border-collapse">
							<thead class="bg-gray-50 border-b border-gray-100 top-0 sticky z-10">
								<tr>
									<th
										class="px-5 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-widest"
									>
										{{ __("Shift Date") }}
									</th>
									<th
									...
									<th
										class="px-5 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-widest text-center"
									>
										{{ __("Times (Open - Close)") }}
									</th>
									<th
										class="px-5 py-3 text-[10px] font-bold text-gray-500 uppercase tracking-widest text-right"
									>
										{{ __("Opening") }}
									</th>
									<th
							...
							<tbody class="divide-y divide-gray-50 bg-white">
								<tr
									...
									class="hover:bg-indigo-50/50 transition-all duration-200 group relative"
								>
									<td
										class="px-5 py-4 font-semibold text-gray-900 group-hover:text-indigo-700"
									>
										{{ formatDate(shift.date) }}
									</td>
									<td class="px-5 py-4">
										<span
											class="px-2 py-1 bg-gray-100 text-gray-700 text-[11px] font-semibold rounded-md uppercase tracking-wide"
										>
											{{ shift.pos_profile }}
										</span>
									</td>
									...
									<td class="px-5 py-4 text-center">
										<div class="flex items-center justify-center gap-2">
											<span
												class="text-[11px] font-semibold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded"
												>{{ formatTime(shift.open_time) }}</span
											>
											<svg
											...
											<span
												v-if="shift.close_time"
												class="text-[11px] font-semibold text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded"
												>{{ formatTime(shift.close_time) }}</span
											>
											<span
												v-else
												class="text-[11px] font-semibold text-green-600 bg-green-50 px-1.5 py-0.5 rounded italic uppercase tracking-tighter"
												>{{ __("Running") }}</span
											>
										</div>
									</td>
									<td
										class="px-5 py-4 text-right tabular-nums font-medium text-gray-600"
									>
									...
									<td
										class="px-5 py-4 text-right tabular-nums font-bold text-gray-900"
									>
									...
									<td class="px-5 py-4 text-right">
										<span
											:class="[
												...
												getCashDiffColor(shift.difference),
											]"
										>
											<svg
												v-if="shift.difference > 0"
												class="w-3 h-3"
												fill="currentColor"
												viewBox="0 0 20 20"
											>
									...
									<td class="px-3 py-4 text-right">
										<div class="flex items-center justify-end gap-1">
											<button
												...
												class="text-[10px] font-semibold px-2 py-1 rounded bg-indigo-50 text-indigo-700 hover:bg-indigo-100 transition-colors"
				...
				>
					<span class="text-[11px] font-medium text-gray-500 select-none">
						...
						{{ (currentPage - 1) * PAGE_SIZE + 1 }}&ndash;{{
							Math.min(currentPage * PAGE_SIZE, serverTotals.total_shifts)
						}}
					...
					<div class="flex items-center gap-1">
						<button
							...
							class="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-white hover:border-indigo-300 hover:text-indigo-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
						>
							...
							<button
								...
								:class="[
									...
									page === currentPage
										...
										: 'border-gray-200 text-gray-600 hover:bg-white hover:border-indigo-300 hover:text-indigo-600 disabled:opacity-50',
								]"
							>
								{{ page }}
							</button>
		...
		<template #actions>
			<Button
				variant="subtle"
				@click="show = false"
				class="font-bold uppercase tracking-widest text-xs px-6"
			>
				{{ __("Close History") }}
			</Button>
		</template>
...
const shiftsResource = createResource({
	url: "DyPOS.api.shifts.get_shift_history",
	makeParams() {
		return {
	...
	onSuccess(response) {
		if (
			...
			serverTotals.value = {
				...
				total_cash_diff: shifts.value.reduce(
