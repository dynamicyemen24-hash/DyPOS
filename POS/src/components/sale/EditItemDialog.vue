											...
											>
												...
												>
													...
													>
														{{ __("Serial Numbers") }}
														<span
															class="ms-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800"
														>
															{{ localSerials.length }}
														</span>
													</label>
												...
												>
													...
													>
														<div class="flex items-center gap-2">
															<span
																class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-indigo-600 text-white text-xs font-medium"
															>
																{{ index + 1 }}
															</span>
															<span
													...
													<div>
														...
														<div class="relative">
															<input
																...
																"
																step="0.01"
																class="w-full h-7 border border-gray-300 rounded-lg px-3 pe-8 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
...
const getItemDetailsResource = createResource({
	url: "DyPOS.api.items.get_item_details",
	auto: false,
})
...
// Accessibility: focus the dialog panel on open, restore focus on close, and
// let the panel handle Escape (role=aria-modal dialog semantics).
...
function handleQuantityInput() {
	...
	if (localQuantity.value > 0 && !Number.isNaN(Number(localQuantity.value))) {
		calculateTotals()
	}
}
...
function handleQuantityBlur() {
	...
	if (
		...
		Number.isNaN(Number(localQuantity.value))
	) {
...
async function getRateForUom(uom) {
	...
	if (localItem.value.item_code && posProfile) {
		try {
			...
			if (!Number.isNaN(serverRate) && serverRate > 0) {
				return serverRate
			}
		} catch (error) {
...
function calculateDiscount() {
	...
	if (
		...
		!Number.isNaN(Number(discountValue.value))
	) {
