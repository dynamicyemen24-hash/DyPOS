										...
										>
											...
											>
													...
													:class="[
														...
														isDraggingImage
															...
															: form.image
																...
																: 'border-dashed border-gray-300 bg-gray-50 hover:border-indigo-400 hover:bg-indigo-50/40',
													]"
													:aria-label="
														form.image
												...
												>
													...
													>
														<div
															class="flex h-16 w-16 items-center justify-center rounded-full bg-white text-gray-400 shadow-sm group-hover:text-indigo-500"
														>
															<FeatherIcon
																name="image"
																class="h-8 w-8"
												...
												<p
													v-if="selectedFile"
													class="mt-2 flex items-center gap-2 text-sm text-indigo-600"
												>
													<FeatherIcon
														name="file"
...
const isFormOpen = computed(
	() => Boolean(selectedProduct.value) || isCreating.value,
)

const allowedExtensions = computed(() => imageSettings.value.extensions || [])
const imagesDisabledBySite = computed(
...
const maxImageSizeLabel = computed(() =>
	formatFileSize(imageSettings.value.max_file_size || 0),
)
...
async function saveProduct() {
	...
	try {
		...
		// 1. Save product
		const result = await call("DyPOS.api.product_management.save_product", {
		...
		if (selectedFile.value) {
			...
			if (fileUrl !== form.value.image) {
				...
				await call("DyPOS.api.product_management.update_product_image", {
					pos_profile: props.posProfile,
					item_code: itemCode,
					file_url: fileUrl,
				})
...
async function loadProducts({ append = false } = {}) {
	...
	try {
		...
		const data = await call("DyPOS.api.product_management.get_products", params)
...
async function loadItemGroups() {
	try {
		const data = await call("DyPOS.api.product_management.get_item_groups", {
			pos_profile: props.posProfile,
		})
...
async function loadImageSettings() {
	try {
		const data = await call(
			"DyPOS.api.product_management.get_product_image_settings",
		)
