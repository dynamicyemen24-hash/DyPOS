			...
			<div class="flex flex-col gap-6">
				...
				<div>
					...
					<div class="flex gap-2">
						...
						<input
							...
							class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 text-start"
				...
				<div>
					...
					<select
						...
						class="w-full px-8 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
					>
				...
				<div>
					...
					<select
						...
						class="w-full px-8 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-gray-100 disabled:cursor-not-allowed"
					>
						<option value="">
							{{
...
const createCustomerResource = createResource({
	url: "DyPOS.api.customers.create_customer",
	makeParams: () => ({
	...
	onSuccess: (data) => {
		...
		if (
			...
			!districts.value.some(
				(d) => d.name === customerData.value.custom_district,
			)
		) {
			customerData.value.custom_district = ""
		}
