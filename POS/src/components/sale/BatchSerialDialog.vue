	...
	>
		<template #body-content>
			<div class="flex flex-col gap-4">
				<!-- Item Info -->
				<div v-if="item" class="bg-indigo-50 rounded-lg p-3">
				...
				<div v-if="item?.has_serial_no">
					...
					<div class="relative mb-3">
						<input
							...
							class="w-full px-3 py-2 ps-9 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
						/>
						<svg
						...
						>
							<div class="flex items-center gap-3">
								...
								<span
									...
									class="flex-shrink-0 inline-flex items-center justify-center w-6 h-6 rounded-full bg-indigo-600 text-white text-xs font-medium"
								>
...
const batchesResource = createResource({
	...
	async onSuccess(data) {
		if (Array.isArray(data?.batch_no_data)) {
