...
describe("retryAsync — إعادة المحاولة بذكاء", () => {
	...
	it("يعيد المحاولة على خطأ شبكي ثم ينجح", async () => {
		...
		const onRetry = vi.fn()
		await expect(retryAsync(fn, { baseDelay: 1, onRetry })).resolves.toBe(
			"synced",
		)
	...
	it("لا يعيد المحاولة على أخطاء العمل (400)", async () => {
		...
		await expect(retryAsync(fn, { baseDelay: 1 })).rejects.toMatchObject({
			status: 400,
		})
		expect(fn).toHaveBeenCalledTimes(1)
	})
	...
	it("يحترم حد المحاولات ثم يرمي آخر خطأ", async () => {
		...
		await expect(
			retryAsync(fn, { retries: 2, baseDelay: 1 }),
