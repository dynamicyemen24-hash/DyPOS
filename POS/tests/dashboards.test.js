...
describe("Finance Dashboard - buildFinanceModels", () => {
	...
	it("computes net cashflow", () => {
		const facts = {
			invoices: [invoice()],
			payments: [
				payment({ payment_type: "Receive", received_amount: 500 }),
				payment({ payment_type: "Pay", paid_amount: 200 }),
			],
			receivables: [],
			payables: [],
			prevInvoices: [],
	...
	it("verifies prevReceivables flows through to receivable KPI", () => {
		const facts = {
			...
			prevPayments: [],
			prevReceivables: [
				receivable({ outstanding_amount: 500, posting_date: "2026-08-01" }),
			],
			prevPayables: [],
		}
		const models = buildFinanceModels(facts)
...
describe("Customers Dashboard - buildCustomerModels", () => {
	it("returns KPIs and top customers", () => {
		const facts = {
			invoices: [
				invoice(),
				invoice({ name: "INV-002", customer: "CUST-2", base_net_total: 2000 }),
			],
			prevInvoices: [prevInvoice()],
		}
...
describe("Operations Dashboard - buildOperationsModels", () => {
	it("returns KPIs and hourly data", () => {
		const facts = {
			invoices: [
				invoice(),
				invoice({ name: "INV-002", base_grand_total: 2000 }),
			],
