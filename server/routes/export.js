...
router.post('/:entity/jobs', (req, res) => {
  ...
  const job = createJob('export', { entity: req.params.entity, format, total, from: String(src.from || '').slice(0, 10), to: String(src.to || '').slice(0, 10), status: String(src.status || '').slice(0, 20) });
  ...
  runInBackground(job, async (j) => {
    ...
    const built = typeof def.args === 'function' && def.args.length ? def.args(q) : { sql: def.sql, args: def.args() };
    const CHUNK = 2000;
    const acc = [];
