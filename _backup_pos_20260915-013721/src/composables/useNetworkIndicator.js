...
function getQualityLevel(info) {
	...
	if (info.effectiveType === "4g" && (info.downlink || 0) >= 5) return "excellent"
	if (info.effectiveType === "4g" || info.effectiveType === "3g") return "good"
	if (info.effectiveType === "2g" || info.effectiveType === "slow-2g") return "poor"
	return "fair"
}
...
export function useNetworkIndicator() {
	...
	const config = computed(() => QUALITY_CONFIG[quality.value] || QUALITY_CONFIG.unknown)
	...
	function startMonitoring() {
		...
		try {
			offlineUnsubscribe = offlineState.subscribe((state) => {
				...
				quality.value = state.isOffline ? "offline" : getQualityLevel(getNetworkInfo())
			})
		} catch {
		}
...
export function createNetworkStatus() {
	...
	return {
		...state,
		config: computed(() => QUALITY_CONFIG[state.quality.value] || QUALITY_CONFIG.unknown),
		update,
