...
export function useInvoice() {
	...
	const updateInvoiceResource = createResource({
		url: "DyPOS.api.invoices.update_invoice",
		makeParams(params) {
	...
	const validateCartItemsResource = createResource({
		url: "DyPOS.api.invoices.validate_cart_items",
	...
	const applyOffersResource = createResource({
		url: "DyPOS.api.invoices.apply_offers",
	...
	const getTaxesResource = createResource({
		url: "DyPOS.api.pos_profile.get_taxes",
		auto: false,
	})

	const getDefaultCustomerResource = createResource({
		url: "DyPOS.api.pos_profile.get_default_customer",
	...
	const cleanupDraftsResource = createResource({
		url: "DyPOS.api.invoices.cleanup_old_drafts",
		auto: false,
	})
		...
		return await submitMutex.withLock(async () => {
			...
			try {
				...
				try {
					...
					if (submitInvoiceResource.error) {
						const resourceError = submitInvoiceResource.error
						log.debug("Resource error details:", {
						...
						if (resourceError.messages && resourceError.messages.length > 0) {
							log.debug("First message:", resourceError.messages[0])
						}
						...
						log.debug("After attaching, error.messages:", error.messages)
					}
	...
	async function setDefaultCustomer() {
		...
		try {
			...
			log.debug("No default customer set in POS Profile")
		}
	}
