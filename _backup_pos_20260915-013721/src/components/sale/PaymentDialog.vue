					...
					>
						...
						>
							...
							<div
								...
								role="switch"
								tabindex="0"
								:aria-checked="applyWriteOff"
								:aria-label="__('Apply write off change')"
								@click="applyWriteOff = !applyWriteOff"
								@keydown.enter.prevent="applyWriteOff = !applyWriteOff"
								@keydown.space.prevent="applyWriteOff = !applyWriteOff"
				...
				>
					...
					<div :class="isSmallMobile ? 'mb-1' : 'mb-1.5 lg:mb-3'">
						...
						<div v-if="loadingPaymentMethods" class="flex items-center gap-2">
							<div
								:class="[
									'animate-spin rounded-full border-b-2 border-indigo-500',
								...
								:class="[
									...
									lastSelectedMethod?.mode_of_payment === method.mode_of_payment
										...
										: isWalletPaymentMethod(method.mode_of_payment)
											...
											: 'border-gray-200 bg-white hover:border-indigo-400 hover:bg-indigo-50 text-gray-700',
								]"
							>
							...
							<template v-if="receivableAccounts.length > 0">
								...
								>
									...
									<span
										...
										:class="[
											'font-bold text-indigo-600 bg-indigo-100 rounded',
											isSmallMobile
												? 'text-[8px] px-0.5 py-0.5'
												: 'text-xs px-1 py-0.5',
										]"
							...
							<button
								...
								:class="[
									...
									isQuickAmountDisabled(amount)
										...
										: 'bg-white border-gray-200 hover:border-indigo-400 hover:bg-indigo-50 text-gray-700 hover:text-indigo-600',
								]"
							>
					...
					<div
						...
						class="hidden lg:block"
						:class="[
							'bg-indigo-50 rounded-lg text-center',
					...
					>
							...
							>
								<button
									...
									:class="[
										...
										isQuickAmountDisabled(amount)
											? 'bg-gray-50 border-gray-100 text-gray-300 cursor-not-allowed'
											: 'bg-white border-gray-200 text-gray-700 active:bg-indigo-50 active:border-indigo-400',
									]"
								>
									{{ formatCurrency(amount) }}
								</button>
								...
								<div class="relative flex-1">
									...
									<input
										...
										:class="[
											...
											!isCashPaymentMethod(lastSelectedMethod)
												? 'bg-gray-50 border-gray-100 text-gray-300 cursor-not-allowed'
												: 'bg-white border-gray-200 focus:ring-1 focus:ring-indigo-500',
										]"
									/>
								</div>
								<button
						...
						<div
							...
							"
							:class="[
								'bg-indigo-50 rounded text-center',
						...
						<div :class="['grid grid-cols-4', isCompactMode ? 'gap-1' : 'gap-1.5']">
							...
							<button
								...
								:class="[
									...
									'text-xl font-semibold rounded-lg bg-gray-50 border-2 border-gray-200 hover:border-indigo-400 hover:bg-indigo-50 text-gray-800 transition-all active:scale-95',
								]"
							>
								{{ num }}
							</button>
							...
							<button
								...
								:class="[
									...
									!numpadValue || numpadValue <= 0 || !lastSelectedMethod
										...
										: 'bg-indigo-600 border-2 border-indigo-600 hover:bg-indigo-700 text-white',
								]"
							>
								{{ __("Add") }}
							</button>
							...
							<button
								...
								:class="[
									...
									'text-2xl font-semibold rounded-xl bg-gray-50 border-2 border-gray-200 hover:border-indigo-400 hover:bg-indigo-50 text-gray-800 transition-all active:scale-95',
								]"
							>
								00
							</button>
							...
							<button
								...
								:class="[
									...
									numpadDisplay.includes('.')
										? 'bg-gray-100 border-2 border-gray-200 text-gray-400 cursor-not-allowed'
										: 'bg-gray-50 border-2 border-gray-200 hover:border-indigo-400 hover:bg-indigo-50 text-gray-800',
								]"
							>
								.
							</button>
						...
						<button
							...
							:class="[
								...
								!canComplete || isSubmitting
									? 'bg-indigo-300 text-white cursor-not-allowed'
									: 'bg-indigo-600 text-white hover:bg-indigo-700 active:bg-indigo-800 focus-visible:ring-2 focus-visible:ring-indigo-400',
							]"
						>
							<svg
								v-if="isSubmitting"
			...
			<div class="p-5">
				...
				<div class="flex gap-2.5 justify-end">
					...
					<button
						...
						class="px-4 py-1.5 rounded-lg text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 transition-colors"
					>
						{{ __("Continue") }}
					</button>
				</div>
...
const paymentMethodsResource = createResource({
	url: "DyPOS.api.pos_profile.get_payment_methods",
	makeParams() {
		return {
...
const receivableAccountsResource = createResource({
	url: "DyPOS.api.pos_profile.get_receivable_accounts",
	makeParams() {
		return {
			pos_profile: props.posProfile,
		}
...
const customerCreditResource = createResource({
	url: "DyPOS.api.credit_sales.get_available_credit",
	makeParams() {
...
const customerBalanceResource = createResource({
	url: "DyPOS.api.credit_sales.get_customer_balance",
	makeParams() {
		const customerName = props.customer?.name || props.customer
...
const walletInfoResource = createResource({
	url: "DyPOS.api.wallet.get_wallet_info",
	makeParams() {
		const customerName = props.customer?.name || props.customer
		log.debug(
...
async function identifyWalletPaymentMethods() {
	...
	try {
		...
		const result = await call(
			"DyPOS.api.pos_profile.get_wallet_payment_flags",
			{
				methods: methodNames,
			},
		)
...
const salesPersonsResource = createResource({
	url: "DyPOS.api.pos_profile.get_sales_persons",
	makeParams() {
		return {
			pos_profile: props.posProfile,
		}
