						...
						<div class="flex-1 relative">
							<div class="relative">
								...
								<input
									...
									:disabled="isOffline"
									:class="[
										'w-full ps-10 pe-10 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500',
										isOffline
							...
							>
								...
								<div
									...
									class="px-3 py-2 flex items-center gap-2 text-indigo-600 bg-indigo-50"
								>
		...
		<template #body-content>
			<div class="flex flex-col gap-4">
				...
				<div
					...
					class="bg-gradient-to-r from-indigo-50 to-indigo-50 rounded-xl p-4 sm:p-5 border border-indigo-100 shadow-sm"
				>
					<!-- Mobile Layout -->
				...
				<div v-if="originalInvoice">
					...
					<div v-if="returnItems.length > 7" class="mb-3 relative">
						<input
							...
							class="w-full px-4 py-2.5 ps-10 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
						/>
						<FeatherIcon
							name="search"
						...
						>
							...
							<div class="hidden sm:flex items-center gap-3">
								...
								<input
									...
									@click.stop
									class="h-4 w-4 text-indigo-600 rounded focus:ring-2 focus:ring-indigo-500 cursor-pointer"
								/>
								...
								>
									...
									<div class="flex items-center gap-1.5">
										...
										<input
											...
											class="w-12 px-1 py-1 border border-gray-300 rounded-lg text-sm text-center font-bold focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
										/>
										<button
											type="button"
							...
							<div class="sm:hidden flex flex-col gap-3">
								...
								<div class="flex items-start gap-3">
									<input
										...
										v-model="item.selected"
										@click.stop
										class="h-5 w-5 mt-1 text-indigo-600 rounded-md focus:ring-2 focus:ring-indigo-500 cursor-pointer flex-shrink-0"
									/>
									<div class="flex-1 min-w-0 text-start">
										<h4
								...
								<div class="flex flex-col gap-2" @click.stop>
									...
									<div class="flex items-center gap-2">
										...
										<input
											...
											@change="normalizeItemQuantity(item)"
											@blur="normalizeItemQuantity(item)"
											class="w-16 h-10 px-2 border-2 border-gray-300 rounded-lg text-lg text-center font-bold focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
										/>
										<button
											@click.stop="incrementReturnQuantity(item)"
											:disabled="
				...
				<div v-if="selectedItems.length > 0">
					...
					<div
						...
						class="bg-indigo-50 rounded-xl p-4 border border-indigo-200 mb-4 text-start"
					>
						...
						<div class="flex flex-col gap-1 text-xs">
							<div class="flex justify-between items-center">
								<span class="text-indigo-700">{{ __("Cash Refund:") }}</span>
							...
							>
								<span class="text-indigo-700">{{ __("Credit Adjustment:") }}</span>
								<span class="font-bold text-indigo-900">{{
							...
							>
								...
								<div class="flex flex-col sm:flex-row sm:items-center gap-3">
									...
									<div class="flex items-center gap-2 flex-1">
										<div
											class="flex-shrink-0 w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-50 to-indigo-100 flex items-center justify-center text-xl border border-indigo-200"
										>
											{{
										...
										<select
											...
											class="payment-select flex-1 py-2.5 border border-gray-300 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white appearance-none cursor-pointer hover:border-gray-400 transition-colors ps-3 pe-10"
										>
									...
									<div class="flex items-center gap-2 flex-1">
										...
										<input
											...
											class="flex-1 min-w-0 px-3 py-2.5 text-base font-bold text-center border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 hover:border-gray-400 transition-colors"
										/>
										<button
				...
				<div v-if="selectedItems.length > 0">
					...
					<textarea
						...
						"
						class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 resize-none"
					></textarea>
				</div>
			</div>
		</template>
...
const searchInvoiceByNumberResource = createResource({
	url: "DyPOS.api.invoices.search_invoice_by_number",
	auto: false,
	onSuccess(data) {
...
const fetchInvoiceResource = createResource({
	url: "DyPOS.api.invoices.prepare_return_invoice",
	auto: false,
	onSuccess(data) {
		if (data) {
...
const checkInvoiceValidityResource = createResource({
	url: "DyPOS.api.invoices.check_invoice_return_validity",
	auto: false,
})
