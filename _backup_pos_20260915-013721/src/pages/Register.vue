...
const passwordStrength = computed(() => {
	...
	if (score <= 2) return { level: score, label: "ضعيف", color: "var(--dy-crimson-600)" }
	if (score <= 3) return { level: score, label: "متوسط", color: "var(--dy-amber-600)" }
...
const passwordError = computed(() => {
	...
	if (password.value.length < 6) return "كلمة المرور يجب أن تكون 6 أحرف على الأقل"
	return ""
})

const confirmPasswordError = computed(() => {
	if (!confirmPassword.value) return ""
	if (confirmPassword.value !== password.value) return "كلمات المرور غير متطابقة"
	return ""
})

const fullNameError = computed(() => {
	if (!fullName.value.trim()) return ""
	if (fullName.value.trim().length < 2) return "الاسم يجب أن يكون حرفين على الأقل"
	return ""
})
